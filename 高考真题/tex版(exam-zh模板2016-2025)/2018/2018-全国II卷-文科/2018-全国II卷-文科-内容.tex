...
\textbf{详解：}∵集合 $A=\{1,3,5,7\}$，$B=\{2,3,4,5\}$，∴$A\cap B=\{3,5\}$。故选：C。
\end{solution}
\end{question}
...
\textbf{详解：}∵双曲线的离心率为 $e=\frac{c}{a}=\sqrt{3}$，则 $\frac{b}{a}=\sqrt{\frac{c^2}{a^2}-1}=\sqrt{3-1}=\sqrt{2}$，即双曲线的渐近线方程为 $y=\pm\frac{b}{a}x=\pm\sqrt{2}x$，故选：A。
\end{solution}
\end{question}
...
\textbf{详解：}∵$f(x)$ 是奇函数，且 $f(1-x)=f(1+x)$，∴$f(1-x)=f(1+x)=-f(x-1)$，$f(0)=0$，则 $f(x+2)=-f(x)$，则 $f(x+4)=-f(x+2)=f(x)$，即函数 $f(x)$ 是周期为 4 的周期函数，∵$f(1)=2$，∴$f(2)=f(0)=0$，$f(3)=f(1-2)=f(-1)=-f(1)=-2$，$f(4)=f(0)=0$，则 $f(1)+f(2)+f(3)+f(4)=2+0-2+0=0$，则 $f(1)+f(2)+f(3)+\cdots+f(50)=12\times0+f(49)+f(50)=f(1)+f(2)=2+0=2$，故选：C。
\end{solution}
\end{question}
...
\textbf{详解：}∵$y=2\ln x$，∴$y'=\frac{2}{x}$，当 $x=1$ 时，$y'=2$，∴切线方程为 $y=2(x-1)$，即 $y=2x-2$。故答案为：$y=2x-2$。
\end{solution}
\end{question}
...
\textbf{详解：}∵$\tan(\alpha-\frac{5\pi}{4})=\frac{1}{5}$，∴$\tan(\alpha-\frac{\pi}{4})=\frac{1}{5}$，则 $\tan\alpha=\tan[(\alpha-\frac{\pi}{4})+\frac{\pi}{4}]=\frac{\tan(\alpha-\frac{\pi}{4})+1}{1-\tan(\alpha-\frac{\pi}{4})}=\frac{\frac{1}{5}+1}{1-\frac{1}{5}}=\frac{\frac{6}{5}}{\frac{4}{5}}=\frac{3}{2}$。故答案为：$\frac{3}{2}$。
\end{solution}
\end{question}
...
\textbf{详解：}（1）∵等差数列 $\{a_n\}$ 中，$a_1=-7$，$S_3=-15$，∴$3a_1+3d=-15$，解得 $d=2$，∴$a_n=-7+2(n-1)=2n-9$；
（2）$S_n=\frac{n(-7+2n-9)}{2}=n^2-8n=(n-4)^2-16$，∴当 $n=4$ 时，前 $n$ 项的和 $S_n$ 取得最小值为 $-16$。
\end{solution}
\end{question}
...
\textbf{详解：}（1）证明：∵$AB=BC=2\sqrt{2}$，$AC=4$，∴$AB^2+BC^2=AC^2$，即 $\triangle ABC$ 是直角三角形，又 $O$ 为 $AC$ 的中点，∴$OA=OB=OC$，∵$PA=PB=PC$，∴$\triangle POA\cong\triangle POB\cong\triangle POC$，∴$\angle POA=\angle POB=\angle POC=90^\circ$，∴$PO\perp AC$，$PO\perp OB$，$OB\cap AC=O$，∴$PO\perp$ 平面 $ABC$；
（2）解：由（1）得 $PO\perp$ 平面 $ABC$，$PO=\sqrt{PA^2-AO^2}=\sqrt{16-4}=2\sqrt{3}$，在 $\triangle COM$ 中，$OM=\sqrt{OC^2+CM^2-2\cdot OC\cdot CM\cdot\cos\angle OCM}$，$S_{\triangle POM}=\frac{1}{2}\times PO\times OM$，$S_{\triangle COM}=\frac{1}{2}\times OC\times CM\times\sin\angle OCM$，设点 $C$ 到平面 $POM$ 的距离为 $d$．由 $V_{P-OMC}=V_{C-POM}\Rightarrow \frac{1}{3}\cdot PO\cdot S_{\triangle OMC}=\frac{1}{3}\cdot d\cdot S_{\triangle POM}$，解得 $d=\frac{4\sqrt{5}}{5}$，∴点 $C$ 到平面 $POM$ 的距离为 $\frac{4\sqrt{5}}{5}$。
\end{solution}
\end{question}
...
\textbf{详解：}（1）方法一：抛物线 $C:y^2=4x$ 的焦点为 $F(1,0)$，设直线 AB 的方程为：$y=k(x-1)$，设 $A(x_1,y_1)$，$B(x_2,y_2)$，联立 $\begin{cases} y=k(x-1) \\ y^2=4x \end{cases}$，整理得：$k^2x^2-2(k^2+2)x+k^2=0$，则 $x_1+x_2=\frac{2(k^2+2)}{k^2}$，$x_1x_2=1$，由 $|AB|=x_1+x_2+p=\frac{2(k^2+2)}{k^2}+2=8$，解得：$k^2=1$，则 $k=1$，∴直线 l 的方程 $y=x-1$；
...
（2）∵$f(x)\leq1$，∴$5-|x+a|-|x-2|\leq1$，∴$|x+a|+|x-2|\geq4$，又 $|x+a|+|x-2|=|x+a|+|2-x|\geq|x+a+2-x|=|a+2|$，∴$|a+2|\geq4$，解得 $a\leq-6$ 或 $a\geq2$，故 $a$ 的取值范围是 $(-\infty,-6]\cup[2,+\infty)$。
\end{solution}
\end{question}
