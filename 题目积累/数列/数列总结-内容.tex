...
（6）$2\dfrac12,3\dfrac23,4\dfrac34,5\dfrac45,\ldots$\qquad
（7）$\dfrac23,\dfrac4{15},\dfrac6{35},\dfrac8{63},\dfrac{10}{99},\ldots$\par
（8）$2,0,2,0,\ldots$\qquad
（9）$\dfrac35,\dfrac12,\dfrac5{11},\dfrac37,\dfrac7{17},\ldots$\par
（10）三角形数\FormulaOCR{image9.wmf}{a_{ n }  =\frac { n(n+1) } { 2 }}\par
（11）正方形数\FormulaOCR{image10.wmf}{a_{ n }  =n ^ { 2 }}\par
（12）谢宾斯基三角形\par
\DocImage{image11.png}{396.95}{111.00}\par
对应的图形个数为$a_n=3^{n-1}$．\par
...
（6）$a_n=(n+1)+\dfrac{n}{n+1}$；\qquad
（7）$a_n=\dfrac{2n}{4n^2-1}$；\par
（8）$a_n=(-1)^{n+1}+1$，或$a_n=1-\cos n\pi$；\par
...
\Needspace{6\baselineskip}
6．（2015•山东理）设数列{\ensuremath{a_{n}}}的前n项和为\ensuremath{S_{n}}，已知2\ensuremath{S_{n}}=\ensuremath{3^{n}}+3．\par
（Ⅰ）求{\ensuremath{a_{n}}}的通项公式；\par
\QuestionSpace{3.5cm}
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
...
\[
 a_{n+1}=\frac12a_n.
\]
因此$\{a_n\}$是首项为$1$、公比为$\dfrac12$的等比数列，从而
\[
 a_2=\frac12,\qquad a_3=\frac14,
\qquad
 \boxed{a_n=\left(\frac12\right)^{n-1}}.
\]
\par\endgroup\fi
...
\Needspace{6\baselineskip}
6．（2015•山东文）已知数列{\ensuremath{a_{n}}}是首项为正数的等差数列，数列
\ensuremath{\left\{\dfrac1{a_na_{n+1}}\right\}}的前n项和为
\ensuremath{\dfrac{n}{2n+1}}．\par
（1）求数列{\ensuremath{a_{n}}}的通项公式；\par
\QuestionSpace{3.5cm}
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
解：（1）设等差数列{\ensuremath{a_{n}}}的首项为\ensuremath{a_{1}}、公差为d，则
\ensuremath{a_1>0}，且
\[
 a_n=a_1+(n-1)d,\qquad a_{n+1}=a_1+nd.
\]
若\ensuremath{d=0}，则题设中的前n项和为\ensuremath{\dfrac{n}{a_1^2}}，
不可能恒等于\ensuremath{\dfrac{n}{2n+1}}，故\ensuremath{d\ne0}。于是
\[
\begin{aligned}
 \sum_{k=1}^{n}\frac1{a_ka_{k+1}}
 &=\frac1d\sum_{k=1}^{n}\left(\frac1{a_k}-\frac1{a_{k+1}}\right)\\
 &=\frac1d\left(\frac1{a_1}-\frac1{a_1+nd}\right)
 =\frac{n}{a_1(a_1+nd)}.
\end{aligned}
\]
由题设得
\[
 \frac{n}{a_1^2+a_1dn}=\frac{n}{2n+1},
\]
比较常数项与n的系数，得到
\[
 a_1^2=1,\qquad a_1d=2.
\]
因为\ensuremath{a_1>0}，所以\ensuremath{a_1=1}，\ensuremath{d=2}，从而
\[
 \boxed{a_n=2n-1}.
\]
\par\endgroup\fi
...
由题意，
\[
a_2-a_1=2,\quad a_3-a_2=3,\quad\ldots,\quad a_n-a_{n-1}=n.
\]
将以上$n-1$个等式相加，得
\[
a_n-a_1=2+3+\cdots+n=\frac{n(n+1)}2-1.
\]
又$a_1=1$，所以
\[
\boxed{a_n=\frac{n(n+1)}2}\qquad(n\in\mathbb N^*).
\]
\par\endgroup\fi
...
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
由$a_n=a_{n-1}\ (n\ge2)$，可知数列从首项起各项都相等．又$a_1=1$，所以
\[
\boxed{a_n=1}\qquad(n\in\mathbb N^*).
\]
\par\endgroup\fi
...
（2）构造等差数列\par
\Needspace{6\baselineskip}
例1．已知数列$\{a_n\}$满足$a_1=1$，且
$a_{n+1}=3a_n+3^n$，求$a_n$．\par
\QuestionSpace{2.5cm}
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
两边同除以$3^n$，得
\[
\frac{a_{n+1}}{3^n}=\frac{a_n}{3^{n-1}}+1.
\]
令$b_n=\dfrac{a_n}{3^{n-1}}$，则$b_{n+1}=b_n+1$，且$b_1=1$，
所以$b_n=n$．因此
\[
\boxed{a_n=n3^{n-1}}.
\]
...
例2 已知数列$\{a_n\}$满足
\[
a_1=3,\qquad a_{n+1}=\frac{6a_n-4}{a_n+2},
\]
求$a_n$．\par
（提示：证明$\left\{\dfrac1{a_n-2}\right\}$是等差数列．）\par
\QuestionSpace{3.5cm}
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
由$a_1=3>2$，且
...
=\frac{4(a_n-2)}{a_n+2},
\]
可归纳得到$a_n>2$，所以以下各式中的分母均不为零．将上式取倒数，得
\[
\frac1{a_{n+1}-2}
=\frac{a_n+2}{4(a_n-2)}
=\frac14+\frac1{a_n-2}.
...
b_n=\frac1{a_n-2},
\]
则$b_{n+1}-b_n=\dfrac14$，且$b_1=1$．因此$\{b_n\}$是首项为$1$、公差为$\dfrac14$的等差数列，
\[
b_n=1+\frac{n-1}{4}=\frac{n+3}{4}.
\]
于是
\[
\frac1{a_n-2}=\frac{n+3}{4},
\qquad
\boxed{a_n=2+\frac4{n+3}=\frac{2(n+5)}{n+3}}.
\]
\par\endgroup\fi
（3）构造等比数列\par
...
a_n+3=2(a_{n-1}+3).
\]
令$b_n=a_n+3$，则$\{b_n\}$是首项$b_1=4$、公比为$2$的等比数列．故
\[
...
（Ⅰ）逐项代入递推式：
\[
a_2=2a_1+\frac23=\frac83,\qquad
a_3=\frac32a_2+\frac24=\frac92,
\]
\[
a_4=\frac43a_3+\frac25=\frac{32}{5}.
\]
（Ⅱ）由
\[
a_{n+1}=\frac{n+1}{n}a_n+\frac2{n+2}
\]
可得
\[
\frac{a_{n+1}}{n+1}-\frac{a_n}{n}
=\frac{2}{(n+1)(n+2)}
=2\left(\frac1{n+1}-\frac1{n+2}\right).
\]
从$1$累加到$n-1$，得到
\[
\frac{a_n}{n}-a_1
=2\left(\frac12-\frac1{n+1}\right).
\]
代入$a_1=1$，得
\[
\frac{a_n}{n}=2-\frac2{n+1}=\frac{2n}{n+1},
\qquad
\boxed{a_n=\frac{2n^2}{n+1}}.
\]
\par\endgroup\fi
...
\par\medskip\noindent{\zihao{4}\bfseries 取倒数法}\par
...
\]
令$b_n=\dfrac{a_n}{2^n}$，则$b_{n+1}-b_n=\dfrac12$，且
$b_1=\dfrac12$．所以$b_n=\dfrac n2$，从而
\[
\boxed{a_n=n2^{n-1}}.
\]
\par\endgroup\fi

\par\medskip\noindent{\zihao{4}\bfseries 取对数法}\par
当递推式中含有幂运算时，可在各项为正的前提下两边取对数，
把乘法或乘方递推转化为一次线性递推．\par
\Needspace{6\baselineskip}
1. 已知数列{\ensuremath{a_{n}}}满足\ensuremath{a_{1}}=3且\ensuremath{a_{n+1}}=\ensuremath{(a_{n}-1)^{2}}+1，求\ensuremath{a_{n}}。\par
\QuestionSpace{2.5cm}
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
由递推式得$a_{n+1}-1=(a_n-1)^2$．由于$a_1-1=2>0$，
可归纳知$a_n-1>0$．令$b_n=\lg(a_n-1)$，则
\[
b_{n+1}=2b_n,\qquad b_1=\lg2.
\]
所以$b_n=2^{n-1}\lg2=\lg\!\left(2^{2^{n-1}}\right)$，从而
\[
\boxed{a_n=2^{2^{n-1}}+1}.
\]
\par\endgroup\fi
...
2．已知数列$\{a_n\}$满足
\[
a_1=1,\qquad a_{n+1}=\frac1c\,a_n^2\quad(c>0),
\]
求数列$\{a_n\}$的通项公式．\par
\QuestionSpace{2.5cm}
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
令$a_n=c^{x_n}$．由$a_1=1$得$x_1=0$，递推式化为
\[
x_{n+1}=2x_n-1.
\]
令$y_n=x_n-1$，则$y_{n+1}=2y_n$，且$y_1=-1$，所以
$x_n=1-2^{n-1}$．因此
\[
\boxed{a_n=c^{\,1-2^{n-1}}
=c\left(\frac1c\right)^{2^{n-1}}}.
\]
当$c=1$时，上式同样成立．\par
\par\endgroup\fi
...
\Needspace{6\baselineskip}
1．数列$\{a_n\}$中，$a_1=1$，对所有的$n\geq2$都有
\[
a_1a_2a_3\cdots a_n=n^2,
\]
则这个数列在$n\geq2$时的通项公式是\underline{\hspace{3cm}}．\par
\QuestionSpace{2.5cm}
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
解析：记
...
\]
由$a_1=1$可知$P_1=1=1^2$；题设又给出$P_n=n^2$（$n\geq2$），
因此$P_n=n^2$对所有正整数$n$均成立．\par
当$n\geq2$时，
\[
a_n=\frac{P_n}{P_{n-1}}
=\frac{n^2}{(n-1)^2}.
...
所以
\[
\boxed{a_n=\left(\frac{n}{n-1}\right)^2\quad(n\geq2)}.
\]
\par
\par\endgroup\fi
...
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
记$S_n=a_1+a_2+\cdots+a_n$．当$n=1$时，
\[
a_1=S_1=3.
\]
当$n\geq2$时，
\[
a_n=S_n-S_{n-1}=(2n+1)-[2(n-1)+1]=2.
\]
所以
\[
...
\[
\frac{b_{n+1}}{n+1}=\frac{b_n}{n}.
\]
因此$\left\{\dfrac{b_n}{n}\right\}$为常数列，又$b_1=1$，故
\[
\boxed{a_n=2^n,\qquad b_n=n}\quad(n\in\mathbb N^*).
\]
\par\endgroup\fi

6．（2018•新课标Ⅰ）已知数列$\{a_n\}$满足$a_1=1$，
$na_{n+1}=2(n+1)a_n$，设$b_n=\dfrac{a_n}{n}$．\par
...
\ensuremath{\therefore}\ensuremath{a_{2}}+\ensuremath{a_{4}}=\ensuremath{a_{1}}+\ensuremath{a_{5}}=6，
故
\[
S_5=\frac52(a_1+a_5)=\frac52\times6=15.
\]
故选B．\par
\par\endgroup\fi
...
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
解：$a_3+a_4+a_5=3a_4=12$，故$a_4=4$．于是
$S_7=7a_4=28$，故选C．\par
\par\endgroup\fi
...
\par\medskip\noindent\textbf{性质3：}若两个等差数列$\{a_n\}$、$\{b_n\}$的前$n$项和分别为$A_n$、$B_n$，则
\[
...
\par\medskip\noindent\textbf{性质1：}若$\{a_n\}$为等比数列，公比为$q$，则每连续$m$项的和组成公比为$q^m$的等比数列；每连续$m$项的积组成公比为$q^{m^2}$的等比数列（各项有定义且不为零）．\par
\Needspace{6\baselineskip}
1.（2013·福建）已知等比数列 $\{a_n\}$ 的公比为 $q$，记\[b_n=\sum_{i=1}^{m}a_{m(n-1)+i},\qquad{}c_n=\prod_{i=1}^{m}a_{m(n-1)+i}\]（$m,n\in N^*$），则以下结论一定正确的是（\quad）。\par
A．$\{b_n\}$为等差数列，公差为$q^m$\qquad{}
B．$\{b_n\}$为等比数列，公比为$q^m$\par
C．$\{c_n\}$为等差数列，公差为$q^{m^2}$\qquad{}
D．$\{c_n\}$为等比数列，公比为$q^{m^2}$\par
\QuestionSpace{1.2cm}
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
相邻两组的每个下标都增加$m$，故
\[
\frac{b_{n+1}}{b_n}=q^m,\qquad
\frac{c_{n+1}}{c_n}=(q^m)^m=q^{m^2}.
\]
因此B、D正确．\par
\par\endgroup\fi

\par\medskip\noindent\textbf{性质2：}若$\{a_n\}$为等比数列，且$m+n=p+q$，则$a_ma_n=a_pa_q$．\par
\Needspace{6\baselineskip}
...
\par\medskip\noindent\textbf{性质3：}若$\{a_n\}$为等比数列，$c\ne0$，则$\{ca_n\}$仍为等比数列．\par
\par\medskip\noindent\textbf{性质4：}若$\{a_n\}$为等比数列，且$a_n^r$恒有意义，则$\{a_n^r\}$仍为等比数列．\par
\par\medskip\noindent\textbf{性质5：}两个非零等比数列逐项相乘、相除，所得数列仍为等比数列．\par
\par\medskip\noindent\textbf{性质6：}若$\{a_n\}$为等比数列，公比$q\ne1$，则每连续$m$项的和组成等比数列，公比为$q^m$．\par
\Needspace{6\baselineskip}
...
\qquad{}\qquad{}\qquad{}A．$5\sqrt2$\qquad{}B．7\qquad{}C．6\qquad{}D．$4\sqrt2$\qquad{}\par
\QuestionSpace{1.2cm}
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
...
因此$a_5^6=a_2^3a_8^3=50$．因各项均为正数，
$a_4a_5a_6=a_5^3=\sqrt{50}=5\sqrt2$，故选A．\par
\par\endgroup\fi

\par\medskip\noindent\textbf{性质8：}若$\{a_n\}$是各项均为正数的等比数列，则$\{\log_ba_n\}$（$b>0,\ b\ne1$）是等差数列．\par
\par\medskip\noindent\textbf{性质9：}当$q>1$时，\par
...
第二组结论对应$0<q<1$．当$q=1$时，该数列为常数列；\par
④当q<0时,该数列为摆动数列.\par
\par\medskip\noindent\textbf{性质10：}等比数列共有$2n$项时，
$S_{\text{偶}}=qS_{\text{奇}}$（相应和有定义）．\par
\par\medskip\noindent\textbf{性质11：等比数列的通项与求和公式}\par
...
\par\medskip\noindent\textbf{性质12：数列的周期性}\par
\qquad{}解决数列周期性问题的方法，先根据已知条件求出数列的前几项，确定数列的周期，再根据周期性求值．\par
\par\medskip\noindent\textbf{性质13：数列的单调性}\par
...
因为$a_7=1>0$而$a_8=-2<0$，所以加入前7项时部分和仍增大，
从第8项起部分和开始减小，故前$n$项和在$n=7$时取得最大值，选B．\par
\par\endgroup\fi

\par\bigskip\noindent{\zihao{3}\bfseries 数列求和}\par
第一节．公式法\par
\qquad{}(1)等差数列$\{a_n\}$的前$n$项和
$S_n=\dfrac{n(a_1+a_n)}2=na_1+\dfrac{n(n-1)}2d$．\par
\qquad{}推导方法：倒序相加法．\par
\qquad{}(2)等比数列$\{a_n\}$的前$n$项和：
$q=1$时$S_n=na_1$；$q\ne1$时$S_n=\dfrac{a_1(1-q^n)}{1-q}$．\par
\qquad{}推导方法：乘公比，错位相减法．\par
\qquad{}(3)一些常见的数列的前n项和：\par
\qquad{}①$1+2+3+\cdots+n=\dfrac{n(n+1)}2$；\par
\qquad{}②2+4+6+\ldots{}+2n＝n(n+1)；\par
\qquad{}③$1+3+5+\cdots+(2n-1)=n^2$．\par
\Needspace{6\baselineskip}
\qquad{}2．求等差数列前n项和\ensuremath{S_{n}}最值的2种方法\par
\qquad{}(1)函数法：利用等差数列前$n$项和的函数表达式
$S_n=An^2+Bn$，通过配方或借助图象求二次函数的最值．\par
\qquad{}(2)邻项变号法：\par
\qquad{}①当$a_1>0,\ d<0$时，若$a_m\geq0$且$a_{m+1}\leq0$，则$S_m$取得最大值；若其中一项等于$0$，相邻两个部分和可能同时取得最大值．\par
\qquad{}②当$a_1<0,\ d>0$时，若$a_m\leq0$且$a_{m+1}\geq0$，则$S_m$取得最小值；同样要单独考虑等于$0$的情形．\par
\qquad{}3．理清等差数列的前n项和与函数的关系\par
\qquad{}等差数列的前$n$项和可写成
\[
S_n=\frac d2n^2+\left(a_1-\frac d2\right)n.
\]
令$A=\dfrac d2$，$B=a_1-\dfrac d2$，则$S_n=An^2+Bn$．\par
...
则$S_{n+1}=-\dfrac13[2+(-2)^{n+2}]$，
$S_{n+2}=-\dfrac13[2+(-2)^{n+3}]$，\par
...
由题意$a_1,a_{11},a_{13}$成等比数列，故
\[
(a_1+10d)^2=a_1(a_1+12d).
\]
代入$a_1=25$并整理，得$100d(d+2)=0$．因为$d\ne0$，所以$d=-2$．\par
...
\[
S_n=a_1+a_4+a_7+\cdots+a_{3n-2}
=\frac{n(a_1+a_{3n-2})}{2}
=\frac{n[25+(-6n+31)]}{2}
=-3n^2+28n.
\]
\par\endgroup\fi
...
（Ⅱ）由（Ⅰ）知$\dfrac1{a_n}=\dfrac2{3^n-1}$．\par
当$n\geq2$时，因$3^n-1>3^n-3^{n-1}$，故
\[
\frac1{a_n}=\frac2{3^n-1}
<\frac2{3^n-3^{n-1}}=\frac1{3^{n-1}}.
\]
...
$|a_1|+|a_2|+\cdots+|a_n|=
\begin{cases}
-\dfrac12n^2+\dfrac{21}{2}n,&n\leq11,\\[3pt]
\dfrac12n^2-\dfrac{21}{2}n+110,&n\geq12.
\end{cases}$\par
\par\endgroup\fi
...
故其前$n$项和为
\[
\left(1-\frac13\right)+\left(\frac13-\frac15\right)+\cdots+
...
\ensuremath{\therefore}
\[
T_n=\left(\frac1{S_1}-\frac1{S_2}\right)+\cdots+
\left(\frac1{S_n}-\frac1{S_{n+1}}\right)
=1-\frac1{2^{n+1}-1}.
\]
\par\endgroup\fi
...
所以数列$\left\{\dfrac{a_n}{n}\right\}$是以1为首项、1为公差的等差数列，\par
...
前$n$项和为
\[
T_n=\frac12\sum_{k=1}^{n}\left(\frac1k-\frac1{k+2}\right)
...
\end{cases}
\]
解得$-\dfrac{13}{3}\leq d\leq-\dfrac{13}{4}$，\par
...
\[
b_n=\frac1{(17-4n)(13-4n)}
=\frac14\left(\frac1{13-4n}-\frac1{17-4n}\right).
\]
因此
\[
T_n=\frac14\left(\frac1{13-4n}-\frac1{13}\right)
=\boxed{\frac{n}{13(13-4n)}}.
\]
\par\endgroup\fi
...
\[
\frac1{S_n+S_{n+1}}
=\frac1{\sqrt{2n}+\sqrt{2n+2}}
=\frac{\sqrt2}{2}\left(\sqrt{n+1}-\sqrt n\right).
\]
所以
\[
\sum_{k=1}^{n}\frac1{S_k+S_{k+1}}
=\boxed{\frac{\sqrt2}{2}\left(\sqrt{n+1}-1\right)}.
\]
\par\endgroup\fi
...
(8). $b_n=\dfrac{n}{2^{n-1}}$，则
$T_n=4-\dfrac{n+2}{2^{n-1}}$．\par
...
（2）求数列$\left\{\dfrac{a_n}{2^n}\right\}$的前$n$项和．\par
\QuestionSpace{3.5cm}
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
...
（Ⅰ）证明：数列$\left\{\dfrac{a_n}{n}\right\}$是等差数列；\par
...
\ensuremath{\therefore}数列$\left\{\dfrac{a_n}{n}\right\}$是以1为首项、以1为公差的等差数列；\par
...
上式对$n=1$也成立，因此
\[
b_{n+1}-b_n=\frac{4n-1}{2^{n-1}}.
\]
从$1$累加到$n-1$，并用错位相减法求和，得
\[
b_n=1+\sum_{k=1}^{n-1}\frac{4k-1}{2^{k-1}}
=\boxed{15-\frac{4n+3}{2^{n-2}}}.
\]
\par\endgroup\fi
...
\[
\frac{b_n}{a_n}
=\left(1-\frac1{2^n}\right)-\left(1-\frac1{2^{n-1}}\right)
=\frac1{2^n},
\]
且$n=1$时也成立．\par
...
\Needspace{6\baselineskip}
例6．（2015•山东理）设数列{\ensuremath{a_{n}}}的前n项和为\ensuremath{S_{n}}，已知2\ensuremath{S_{n}}=\ensuremath{3^{n}}+3．\par
（Ⅰ）求{\ensuremath{a_{n}}}的通项公式；\par
...
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
解：（Ⅰ）令$n=1$，由$2S_1=3+3$，得$a_1=3$．\par
当$n\geq2$时，
\[
2a_n=2S_n-2S_{n-1}
=\left(3^n+3\right)-\left(3^{n-1}+3\right)
=2\cdot3^{n-1},
\]
所以$a_n=3^{n-1}$．因此
\[
\boxed{a_n=\begin{cases}3,&n=1,\\3^{n-1},&n\geq2.\end{cases}}
\]
（Ⅱ）由$a_nb_n=\log_3a_n$，得
\[
b_n=\frac{\log_3a_n}{a_n}
=\begin{cases}\dfrac13,&n=1,\\[3pt]\dfrac{n-1}{3^{n-1}},&n\geq2.\end{cases}
\]
于是
\[
T_n=\frac13+\sum_{k=1}^{n-1}\frac{k}{3^k}.
\]
利用有限等差--等比数列求和公式，
\[
\sum_{k=1}^{n-1}\frac{k}{3^k}
=\frac34-\frac{2n+1}{4\cdot3^{n-1}},
\]
故
\[
\boxed{T_n=\frac{13}{12}-\frac{2n+1}{4\cdot3^{n-1}}}.
\]
当$n=1$时，上式也给出$T_1=\frac13$，故对所有正整数$n$成立．\par
\par\endgroup\fi
...
（Ⅰ）求数列{\ensuremath{a_{n}}}的通项公式；\par
（Ⅱ）设$b_n=2^{a_n-2}+n$，求$b_1+b_2+\cdots+b_{10}$的值．\par
\QuestionSpace{4.8cm}
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
...
\Needspace{6\baselineskip}
例2．（2014•湖南）已知数列$\{a_n\}$的前$n$项和
$S_n=\dfrac{n^2+n}{2}$，$n\in\mathbb N^*$．\par
（Ⅰ）求数列{\ensuremath{a_{n}}}的通项公式；\par
（Ⅱ）设$b_n=2^{a_n}+(-1)^na_n$，求数列$\{b_n\}$的前$2n$项和．\par
\QuestionSpace{4.8cm}
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
解：（Ⅰ）当n=1时，\ensuremath{a_{1}}=\ensuremath{s_{1}}=1，\par
当$n\ge2$时，$a_n=S_n-S_{n-1}=n$，\par
...
（1）求{\ensuremath{a_{n}}}的通项公式；\par
（2）设$c_n=a_n+b_n$，求数列$\{c_n\}$的前$n$项和．\par
\QuestionSpace{3.5cm}
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
...
\[
\sum_{k=1}^nS_k
=\frac{2a_1}{3}\left[n-\sum_{k=1}^n\left(-\frac12\right)^k\right].
\]
而
\[
\sum_{k=1}^n\left(-\frac12\right)^k
=-\frac13\left[1-\left(-\frac12\right)^n\right]<0,
\]
故$\sum_{k=1}^nS_k>\dfrac{2na_1}{3}$，D正确．因此选A、C、D．\par
...
\Needspace{6\baselineskip}
27．（2022•新高考Ⅰ）记$S_n$为数列$\{a_n\}$的前$n$项和，已知
$a_1=1$，$\left\{\dfrac{S_n}{a_n}\right\}$是公差为$\dfrac13$的等差数列．\par
（1）求$\{a_n\}$的通项公式；\par
（2）证明：$\dfrac1{a_1}+\dfrac1{a_2}+\cdots+\dfrac1{a_n}<2$．\par
\QuestionSpace{2.5cm}
\ifshowanswers
\par\begingroup\color{solutioncolor}\noindent\textbf{解答}\quad
