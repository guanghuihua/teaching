...
\subquestion{（1）}求点 $P$ 的轨迹方程；
\subquestion{（2）}设点 $Q$ 在直线 $x=-3$ 上，且 $\overrightarrow{OP}\cdot\overrightarrow{PQ}=1$．证明：过点 $P$ 且垂直于 $OQ$ 的直线 $l$ 过 $C$ 的左焦点 $F$．
\begin{solution}
...
（2）证明：设 $Q(-3,m)$，$P(\sqrt{2}\cos\alpha,\sqrt{2}\sin\alpha)$（$0\le\alpha<2\pi$），$\overrightarrow{OP}\cdot\overrightarrow{PQ}=1$，可得 $(\sqrt{2}\cos\alpha,\sqrt{2}\sin\alpha)\cdot(-3-\sqrt{2}\cos\alpha,m-\sqrt{2}\sin\alpha)=1$，即为 $-3\sqrt{2}\cos\alpha-2\cos^2\alpha+\sqrt{2}m\sin\alpha-2\sin^2\alpha=1$，当 $\alpha=0$ 时，上式不成立，则 $0<\alpha<2\pi$，解得 $m=\frac{3(1+\sqrt{2}\cos\alpha)}{\sqrt{2}\sin\alpha}$，即有 $Q(-3,\frac{3(1+\sqrt{2}\cos\alpha)}{\sqrt{2}\sin\alpha})$，椭圆 $\frac{x^2}{2}+y^2=1$ 的左焦点 $F(-1,0)$，由 $\overrightarrow{PF}\cdot\overrightarrow{OQ}=(-1-\sqrt{2}\cos\alpha,-\sqrt{2}\sin\alpha)\cdot(-3,\frac{3(1+\sqrt{2}\cos\alpha)}{\sqrt{2}\sin\alpha})=3+3\sqrt{2}\cos\alpha-3(1+\sqrt{2}\cos\alpha)=0$．可得过点 $P$ 且垂直于 $OQ$ 的直线 $l$ 过 $C$ 的左焦点 $F$．
...
\subquestion{（1）}求椭圆的标准方程；

\subquestion{（2）}已知动点 $P$ 不在 $y$ 轴上，点 $R$ 在射线 $AP$ 上，且满足 $AR \cdot AP = 3$.

\subsubquestion{（i）}设 $P(m,n)$，求点 $R$ 的坐标（用 $m$，$n$ 表示）；

\subsubquestion{（ii）}设 $O$ 为坐标原点，$M$ 是椭圆上的动点，直线 $OR$ 的斜率为直线 $OP$ 的斜率的 $3$ 倍，求 $|PM|$ 的最大值.
\begin{solution}
...
\subquestion{（I）}证明$|EA|+|EB|$为定值，并写出点$E$的轨迹方程；
\subquestion{（II）}设点$E$的轨迹为曲线$C_1$，直线$l$交$C_1$于$M$，$N$两点，过$B$且与$l$垂直的直线与圆$A$交于$P$，$Q$两点，求四边形$MPNQ$面积的取值范围．
\begin{solution}
...
\begin{question}
\subquestion{（1）}团队在 $O$ 点西侧、东侧20千米处设有 $A$、$B$ 两站点，测量距离发现一点 $P$ 满足 $|PA|-|PB|=20$ 千米，可知 $P$ 在 $A$、$B$ 为焦点的双曲线上，以 $O$ 点为原点，东侧为 $x$ 轴正半轴，北侧为 $y$ 轴正半轴，建立平面直角坐标系，$P$ 在北偏东 $60^\circ$ 处，求双曲线标准方程和 $P$ 点坐标．
\subquestion{（2）}团队又在南侧、北侧15千米处设有 $C$、$D$ 两站点，测量距离发现 $|QA|-|QB|=30$ 千米，$|QC|-|QD|=10$ 千米，求 $|OQ|$（精确到1米）和 $Q$ 点位置（精确到1米，$1^\circ$）
\begin{solution}
\textbf{答案：}$(1)\ \frac{x^2}{100}-\frac{y^2}{300}=1,\ P(\frac{15\sqrt{2}}{2},\frac{5\sqrt{6}}{2})$；$(2)\ |OQ|\approx 19.227$ 千米（约 $19227$ 米），$Q$ 点位置北偏东 $66^\circ$

\textbf{详解：}（1）由题意可得 $a=10$，$c=20$，所以 $b^2=300$，所以双曲线的标准方程为 $\frac{x^2}{100}-\frac{y^2}{300}=1$，直线 $OP:y=\frac{\sqrt{3}}{3}x$，联立双曲线方程，可得 $x=\frac{15\sqrt{2}}{2}$，$y=\frac{5\sqrt{6}}{2}$，即点 $P$ 的坐标为 $(\frac{15\sqrt{2}}{2},\frac{5\sqrt{6}}{2})$．\newline（2）由 $|QA|-|QB|=30$，得 $a=15$，$c=20$，$b^2=175$，双曲线方程为 $\frac{x^2}{225}-\frac{y^2}{175}=1$；由 $|QC|-|QD|=10$，得 $a=5$，$c=15$，$b^2=200$，双曲线方程为 $\frac{y^2}{25}-\frac{x^2}{200}=1$．两双曲线方程联立，并由距离差的符号确定 $Q$ 在第一象限，得 $Q\left(\sqrt{\frac{14400}{47}},\sqrt{\frac{2975}{47}}\right)$，因此 $|OQ|=\sqrt{\frac{17375}{47}}\text{ 千米}\approx19.227\text{ 千米}$（约 $19227$ 米），$Q$ 点位置为北偏东 $66^\circ$．
...
\subquestion{（1）}若 $\triangle POF_2$ 为等边三角形，求 $C$ 的离心率；
\subquestion{（2）}如果存在点 $P$，使得 $PF_1\perp PF_2$，且 $\triangle F_1PF_2$ 的面积等于 $16$，求 $b$ 的值和 $a$ 的取值范围．
\begin{solution}
...
\subquestion{（1）}求 $C$ 的方程，并说明 $C$ 是什么曲线；
\subquestion{（2）}过坐标原点的直线交 $C$ 于 $P$，$Q$ 两点，点 $P$ 在第一象限，$PE\perp x$ 轴，垂足为 $E$，连结 $QE$ 并延长交 $C$ 于点 $G$．
\subsubquestion{（i）}证明：$\triangle PQG$ 是直角三角形；
\subsubquestion{（ii）}求 $\triangle PQG$ 面积的最大值．
\begin{solution}
...
\subquestion{（1）}求 $C$ 的方程；
\subquestion{（2）}设直线 $l$ 不经过 $P_2$ 点且与 $C$ 相交于 $A$，$B$ 两点．若直线 $P_2A$ 与直线 $P_2B$ 的斜率的和为 $-1$，证明：$l$ 过定点．
\begin{solution}
...
\subquestion{（1）}求$E$的方程；
\subquestion{（2）}证明：直线$CD$过定点。
\begin{solution}
...
\subquestion{（1）}求$p$；
\subquestion{（2）}若点$P$在$M$上，$PA$，$PB$是$C$的两条切线，$A$，$B$是切点，求$\triangle PAB$面积的最大值。
\begin{solution}
...
\subquestion{（1）}证明：直线 $AB$ 过定点；
\subquestion{（2）}若以 $E\left(0,\dfrac{5}{2}\right)$ 为圆心的圆与直线 $AB$ 相切，且切点为线段 $AB$ 的中点，求四边形 $ADBE$ 的面积。
\begin{solution}
...
\subquestion{（1）}证明：直线AB过定点；
\subquestion{（2）}若以$E(0,\frac{5}{2})$为圆心的圆与直线AB相切，且切点为线段AB的中点，求该圆的方程。
\begin{solution}
...
\subquestion{（I）}若 $F$ 在线段 $AB$ 上, $R$ 是 $PQ$ 的中点, 证明 $AR\parallel FQ$;
\subquestion{（II）}若 $\triangle PQF$ 的面积是 $\triangle ABF$ 的面积的两倍, 求 $AB$ 中点的轨迹方程.
\begin{solution}
\textbf{答案：}（I）证明见详解；（II）设 $AB$ 中点为 $(x,y)$，其轨迹为 $y^2=x-1$ 或 $y^2=x\ (y\ne0)$．

\textbf{详解：}（I）设 $A\left(\frac{u^2}{2},u\right)$，$B\left(\frac{v^2}{2},v\right)$，则 $P\left(-\frac12,u\right)$，$Q\left(-\frac12,v\right)$，$R\left(-\frac12,\frac{u+v}{2}\right)$．由 $F\left(\frac12,0\right)$ 在线段 $AB$ 上，得弦 $AB$ 的横截距为 $\frac12$，即 $uv=-1$．于是 $k_{AR}=\frac{u-v}{1+u^2}=\frac1u=-v=k_{FQ}$，故 $AR\parallel FQ$．

（II）设 $A(x_1,y_1)$，$B(x_2,y_2)$，直线 $AB$ 与 $x$ 轴交于 $N(x_N,0)$，且 $AB$ 中点为 $M(x,y)$．由 $F\left(\frac12,0\right)$、准线 $x=-\frac12$，得 $S_{\triangle PQF}=\frac12|y_1-y_2|$，$S_{\triangle ABF}=\frac12|FN||y_1-y_2|$．面积条件给出 $|FN|=\frac12$，故 $x_N=0$ 或 $x_N=1$．又 $(y_1-y_2)(y_1+y_2)=2(x_1-x_2)$，所以 $AB$ 的斜率为 $\frac1y$；而 $MN$ 的斜率为 $\frac{y}{x-x_N}$．因此 $y^2=x-x_N$．当 $x_N=1$ 时，轨迹为 $y^2=x-1$；当 $x_N=0$ 时，轨迹为 $y^2=x$，且两条平行线不同，故 $y\ne0$．
...
\subquestion{（1）}求$C_1$的离心率；
\subquestion{（2）}设$M$是$C_1$与$C_2$的公共点，若$|MF|=5$，求$C_1$与$C_2$的标准方程.
\begin{solution}
...
\subquestion{（I）}求椭圆的方程；
\subquestion{（II）}直线 $l$ 与椭圆有唯一的公共点 $M$，与 $y$ 轴的正半轴交于点 $N$，过 $N$ 与 $BF$ 垂直的直线交 $x$ 轴于点 $P$．若 $MP // BF$，求直线 $l$ 的方程．
\begin{solution}
...
\textbf{详解：}因为双曲线$\frac{x^2}{a^2}-\frac{y^2}{b^2}=1(a>0,b>0)$的离心率为$2$，所以$e^2=\frac{c^2}{a^2}=\frac{a^2+b^2}{a^2}=4$，所以$\frac{b^2}{a^2}=3$，所以该双曲线的渐近线方程为$y=\pm\frac{b}{a}x=\pm\sqrt{3}x$.故答案为：$y=\pm\sqrt{3}x$.
...
\subquestion{（1）}求椭圆方程.
\subquestion{（2）}过点 $(0,-\frac{3}{2})$ 的动直线与椭圆有两个交点 $P$, $Q$. 在 $y$ 轴上是否存在点 $T$ 使得 $\overrightarrow{TP}\cdot\overrightarrow{TQ}\leq 0$ 恒成立. 若存在求出这个 $T$ 点纵坐标的取值范围, 若不存在请说明理由.
\begin{solution}
...
\subquestion{（1）}求$C$的方程；
\subquestion{（2）}记$C$的左、右顶点分别为$A_{1}$，$A_{2}$，过点$(-4,0)$的直线与$C$的左支交于$M$，$N$两点，$M$在第二象限，直线$MA_{1}$与$NA_{2}$交于点$P$．证明:点$P$在定直线上.
\begin{solution}
...
\subquestion{（1）}求$E$的方程；
\subquestion{（2）}设过点$P(1,-2)$的直线交$E$于$M,N$两点，过$M$且平行于$x$轴的直线与线段$AB$交于点$T$，点$H$满足$\overrightarrow{MT}=\overrightarrow{TH}$．证明：直线$HN$过定点．
\begin{solution}
