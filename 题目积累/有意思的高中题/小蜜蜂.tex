...
\[
 a_1=3,\qquad
 a_{n+1}=\frac{6a_n-4}{a_n+2}\quad(n\in\mathbb N^*).
\]
求数列 $\{a_n\}$ 的通项公式。
...
\[
 ...
 a_4=\frac{18}{7},\ldots
\]
这些数本身的规律并不明显，但它们都大于 $2$，并且似乎逐渐靠近
$2$。这提示我们：与其只问“每一项是多少”，不如先问
...
不动点是递推运动中不变的参照物。原来的 $a_n$ 是以 $0$ 为原点
描述的绝对位置，而
\[
 a_n-2
\]
描述的是第 $n$ 项与不动点 $2$ 之间的距离。研究这个距离，可以把
“不变的中心”和“真正变化的部分”分开。

\medskip
\textbf{三、为什么要减去 $2$}

因为 $f(2)=2$，所以 $f(x)-2$ 在 $x=2$ 时为零，其分子一定含有
因式 $x-2$。事实上，
\[
\begin{aligned}
 a_{n+1}-2
...
则 $a_n+2=x_n+4$，于是
\[
 x_{n+1}=\frac{4x_n}{x_n+4}.
\]
这一步不是凭经验随意“减去一个数”，而是把坐标原点平移到了
递推变换自身的中心。
...
\]
也就是说，数列始终位于不动点 $2$ 的上方，并不断向它靠近。

还可以不依赖通项公式直接观察这一点。因为 $x_1=a_1-2=1>0$，
并且
\[
 \frac{x_{n+1}}{x_n}=\frac4{x_n+4}\in(0,1),
\]
所以 $0<x_{n+1}<x_n$。这说明“与不动点的距离”不断缩小。
...
不动点不是为了制造一个需要减去的数，而是为了找到递推变换最自然
的坐标原点。原坐标告诉我们“数列在哪里”，新坐标告诉我们
“数列怎样运动”。
\end{solution}
\end{question}
