...
\section{圆锥曲线大题}
	...
	\begin{enumerate}[label=\arabic*.]
		\item \textbf{几何建模能力}
		\newline—— 把文字条件转化为点、线、动点轨迹的代数约束。
		\newline比如：``点 P 在椭圆上且满足 $PF_1 = 2PF_2$''，立刻写出椭圆方程 + 距离公式建立方程组。
		
		\item \textbf{代数结构控制能力}
		\newline—— 用参数法、消元、判别式、取最值等手段``降维''。
		\newline如：设动点坐标 $(x, y)$，转化成一个单变量函数，最后借助判别式或一元二次方程解的存在性解决问题。
		
		\item \textbf{数形结合与极值意识}
		\newline—— 关键突破口几乎都来自``图像关系''或``几何意义''。
		\newline比如：``点到定直线距离最小''，你若代入距离公式死算，极容易错；换成几何对称反射思想，立刻变简单。
	\end{enumerate}
	...
	\begin{table}[h]
		...
		\begin{tabular}{|p{2.5cm}|p{4cm}|p{5.5cm}|}
			...
			\hline
			\textbf{建系定参} & 先定坐标系，确定参数 $a$、$b$、$e$。 & 若题中提及``焦点''、``准线''、``离心率''等，\textbf{优先选标准方程}。否则，可用一般式 $Ax^2 + Bxy + Cy^2 + Dx + Ey + F = 0$。 \\
			\hline
			\textbf{② 写出条件方程} & 将动点或线段的几何条件翻译成代数式。 & 常用套路：点在线上 $\rightarrow$ 代入直线方程；到焦点距离关系 $\rightarrow$ 根号公式；共圆/共线 $\rightarrow$ 行列式 = 0。 \\
			\hline
			\textbf{化简/参数化} & 引入参数使问题降维。 & 典型例子：椭圆 $\frac{x^2}{a^2} + \frac{y^2}{b^2} = 1$ 可设 $x = a\cos\theta$, $y = b\sin\theta$；双曲线设 $x = a\sec\theta$, $y = b\tan\theta$。 \\
			\hline
			\textbf{判定与优化} & 研究方程有解条件、最值、范围。 & 主要武器：判别式、均值不等式、导数或几何极值。 \\
			\hline
			\textbf{⑤回代与解释} & 代回原式并说明几何意义。 & 常出错点：没有说明``取等时的几何条件''，导致逻辑不完整。 \\
			\hline
		\end{tabular}
	\end{table}
