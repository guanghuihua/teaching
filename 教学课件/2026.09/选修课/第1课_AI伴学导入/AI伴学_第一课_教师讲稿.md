...
> 操作与备课：R13。模型达到金牌标准，不是作为学生参赛获得奖牌。这一页只作过渡，不展开竞赛解法。


## 第13页　NS方程：用数学描述水怎样流动
...
> 操作与备课：R8、R10。用手势表示不同位置的流速即可，不做真实流体实验。杯中水是引入情境，不是论文研究区域；黏性不是所有流速都单调减小的保证。不引入偏导数、散度或拉普拉斯算子。


## 第14页　难在哪里：一直算下去，会发生什么
...
> 操作与备课：留约10秒让学生口算。正确答案10000。只讨论t从小于1的一侧趋近1；t=1不在定义域。光滑性问题比单个速度值是否有界更一般，这里用速度爆破介绍新论文涉及的一种情形。湍流或普通漩涡不等于数学奇异性。
...
> 操作与备课：R8—R10。2026-09-11复查作者公布页和Clay页面；后者仍列Active。精确条件是三维不可压、有适当光滑外力、初始静止且动能有界。课堂不讲C、D表述、能量估计或Lean语法；没有独立复核论文和代码，不宣布正式获奖或已获普遍认可。
...
> 操作与备课：Library→Open reader→定位预选页面→选中一句话→Explain/Check me或输入课件提示。约3分钟真实操作与原文对照，留1分钟请学生用自己的话复述或提出追问。响应超过30秒，打开课前已有记录并说明这是以前保存的对话。
...
> 操作与备课：Practice & Prove→Attempt→Save attempt→Request one hint→保存修改→Schedule 1d + 7d reviews→Reports。约4分钟真实操作，留1分钟请学生判断提示是否解决了卡点。到39分钟必须回第19页；只有提前完成，才点集合题。
...
- [R8：NS研究公布说明](https://openai.com/index/navier-stokes-solution/)

- [R9：作者的Lean代码与构建说明](https://github.com/openai/NavierStokesAndEuler)

- [R10：Clay的NS问题页面](https://www.claymath.org/millennium/navier-stokes-equation/)

- [R13：2025年IMO金牌标准](https://deepmind.google/blog/advanced-version-of-gemini-with-deep-think-officially-achieves-gold-medal-standard-at-the-international-mathematical-olympiad/)
