...
> 操作与备课：依据《教学内容（暂定）.docx》主表第1—10课时归纳。用约1分钟介绍主题，不现场解释复数、一笔画条件或分形定义。前两课内容仍按当前课件及后续配置课执行，不照搬文档补充页的旧第一课时间表。
...
> 操作与备课：依据主表第11—18课时归纳。第17课侧重方法成果展示，第18课包含分层独立任务与新主题计划；新主题计划不等于已经掌握。两页介绍与后面的40/30/30评价衔接，不新增考试范围或提交要求。
...
> 操作与备课：Library→Open reader→定位预选页面→选中一句话→Explain/Check me或输入课件提示。约2分钟真实操作与原文对照，留1分钟请学生用自己的话复述或提出追问。响应超过30秒，打开课前已有记录并说明这是以前保存的对话。
...
> 操作与备课：Practice & Prove→Attempt→Save attempt→Request one hint→保存修改→Schedule 1d + 7d reviews→Reports。约3分钟真实操作，留1分钟请学生判断提示是否解决了卡点。到39分钟必须回第21页；只有提前完成，才点集合题。
...
> 操作与备课：优先只展示22—23页，完整解答供下次课。题面由原印刷照片整理，照片本身不放进课件。
